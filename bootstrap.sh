...
apt-get install -y make vim curl python-software-properties build-essential unzip rpm > /dev/null 2>&1
apt-get install -y debconf-utils git dbconfig-common > /dev/null 2>&1
...
# fixes the default document root to /var/www
...
echo 'Application directory "web/" selected as DocumentRoot'
fi
...
apt-get install -y php5 libapache2-mod-php5 php5-cli php5-mcrypt php5-gd php5-ldap > /dev/null 2>&1
apt-get install -y php5-curl php5-common php5-fpm php5-curl > /dev/null 2>&1
